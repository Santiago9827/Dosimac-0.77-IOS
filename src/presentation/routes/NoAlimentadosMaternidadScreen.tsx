...
import {
    consultarAnimalesNoAlimentadosMaternidad, consultarNumeroAnimalesMaternidad,
                ...
                <View style={styles.accionesTarjeta}>
                    ...
                    >
                        ...
                        <Text style={styles.textoBotonVerEstado}>
                            {t('noAlimentadosMaternidad.viewStatus')}
                        </Text>
                    </TouchableOpacity>
                </View>
            </View>
...
export const NoAlimentadosMaternidadScreen = () => {
    ...
    const abrirDetalleAnimal = useCallback(
        async (item: AnimalNoAlimentado) => {
            ...
            const pkidNumero = Number(item.animal?.id ?? item.id ?? 0);

            const pkidValido =
                Number.isFinite(pkidNumero) && pkidNumero > 0
                    ? pkidNumero
                    : null;

            if (!animalId && !pkidValido) {
                Alert.alert(
                    t('noAlimentadosMaternidad.error'),
                    t('noAlimentadosMaternidad.animalIdNotFound'),
                );
            ...
            try {
                ...
                if (animalId) {
                    datosApi = await consultarMaternidadPorId(animalId);
                } else if (pkidValido) {
                    datosApi = await consultarMaternidadPorPkid(pkidValido);
                }
                ...
                navigation.navigate('EstadoAnimalDetalle', {
                    ...
                    origen: 'noAlimentados',
                });
            } catch (error: any) {
                console.log('Error abriendo detalle animal:', error);

                Alert.alert(
                    t('noAlimentadosMaternidad.error'),
                    error?.message ??
                    t('noAlimentadosMaternidad.openAnimalInfoError'),
                );
            }
        },
        [navigation, t],
