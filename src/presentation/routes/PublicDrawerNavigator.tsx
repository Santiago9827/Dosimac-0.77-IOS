...
const PublicDrawerContent = (props: DrawerContentComponentProps) => {
  ...
  const activeRoute = props.state.routeNames[props.state.index]; // Login / PublicHome / Register / FarmList / Settings
  ...
  const isOnLogin = activeRoute === 'Login';
      ...
      {isOnLogin ? (
        <>
          ...
          <DrawerItem
            ...
            onPress={() => go('PublicHome')}
          />
        </>
      ) : (
        <>
          {/* En “principal pública”: Login + Alta Dosimac + Instalaciones */}
          <DrawerItem
            label="Login"
            icon={() => <IonIcon name="log-in-outline" color={iconColor(activeRoute === 'Login')} />}
            labelStyle={labelStyle(activeRoute === 'Login')}
