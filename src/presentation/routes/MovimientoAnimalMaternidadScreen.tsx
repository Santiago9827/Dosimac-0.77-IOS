...
export const MovimientoAnimalMaternidadScreen = () => {
    ...
    const [modalMensajeTipo, setModalMensajeTipo] = useState<'success' | 'error'>('error');
    const [modalMensajeTitulo, setModalMensajeTitulo] = useState('');
    const [modalMensajeTexto, setModalMensajeTexto] = useState('');

    const [tipoBusquedaSalida, setTipoBusquedaSalida] = useState<BusquedaTipo>('corral');
    ...
    const confirmarMovimiento = async () => {
        ...
        if (movimiento === 'entrada') {
            ...
            try {
                ...
                const endpoint = await construirEndpointAppV1('maternity');
                ...
                if (!respuesta.ok) {
                    const mensajeBackend = obtenerMensajeBackend(texto, respuesta.status);
        ...
        const valorLimpio = valorSalida.trim();
        ...
        if (tipoBusquedaSalida === 'corral') {
            const corralNumero = Number(valorLimpio);
            ...
            rutaSalida = `maternity/exitByCorral/${corralNumero}`;
        } else {
            rutaSalida = `maternity/exitById/${encodeURIComponent(valorLimpio)}`;
        }

        try {
            setEnviandoMovimiento(true);

            const endpoint = await construirEndpointAppV1(rutaSalida);
                            ...
                            <>
                                ...
                                <View style={styles.searchOptions}>
                                    ...
                                    <TouchableOpacity
                                        activeOpacity={0.9}
                                        onPress={() => {
                                            setTipoBusquedaSalida('id');
                                            setValorSalida('');
                                        }}
                                        style={[
                                            styles.searchOption,
                                            tipoBusquedaSalida === 'id' && {
                                                borderColor: PURPLE,
                                                backgroundColor: '#F5F3FF',
                                            },
                                        ]}
